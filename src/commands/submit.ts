...
export default class Submit {
  command: any = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('challenge_id').setDescription('ID for the challenge you want to submit to (e.g. ECP2-47)').setRequired(true))
    .addStringOption(option => option.setName('url').setDescription('URL to your work').setRequired(true))
  ...
  async execute(interaction: Discord.CommandInteraction) {
    ...
    await guild.channels
      .cache
      .get(process.env.BOT_MESSAGES_CHANNEL_ID)
      .send({
