...
export default class Mention implements ICommandMention, IContextMenuMention {
  ...
  async createMention(
    ...
    if (helpMention && helpMention.channel && helpMention.message) {
      ...
      // otherwise look for last message sent in the channel by the user
      ...
      if (question) {
        ...
        await interaction.editReply({
          content:
            'You have not sent a message recently. Try send your question and re-try this command.',
        });
    ...
    await (<Discord.TextChannel>(
      guild.channels.cache.get(process.env.MENTION_LOG_CHANNEL_ID)
  ...
  async sendMention(
    ...
    if (new Date(Date.now()) >= helpDate) {
      ...
      const h = await (<Discord.TextChannel>(
        guild.channels.cache.get(helpChannel)
      )).send({
        content: `${helpUserMention}`,
        embeds: [m],
        ...(helpMessage.attachments &&
          helpMessage.attachments.toJSON().length > 0 && {
            files: helpMessage.attachments.toJSON(),
          }),
      });
  ...
  async execute(interaction: Discord.ChatInputCommandInteraction) {
    ...
    if (options.getSubcommand() === 'create') {
      ...
      if (options.getRole('role')) {
        ...
        mention = guild.roles.cache.find(
          ({ name }) =>
            name.toLowerCase() ===
            channel.name
              .replace(/-[a-z]$/, '')
      ...
      if (helpMention && helpMention.mention) {
        await interaction.editReply({
          content:
            'You already have a mention created. Use `/mention overwrite` to create a new one.',
        });
      ...
      if (!helpMention || !helpMention.mention) {
        await interaction.editReply({
          content:
            'You do not have any mention keys created. To create one, use `/mention create`.',
        });
